  	...
  	DCID=`cat serverlist.log | grep -E "\bDCID\b|\bSUBID\b" | sed -n "/$MONITOR_SUBID/{x;p};h" | awk -F '"' '{print$4}'`
        ...
        then
            ...
            then
                ...
                curl -H "$key" https://api.vultr.com/v1/server/create --data "DCID=$DCID" --data 'VPSPLANID=201' --data 'OSID=193'  --data "label=$LABEL" --data "SSHKEYID=$SSHKEYID" > subid.txt
		...
		sleep 20
                ...
                echo `date +"%Y-%m-%d %H:%M:%S"` "流量小于50G，监控结束！"
            fi
        fi
        echo "                                                                              "
